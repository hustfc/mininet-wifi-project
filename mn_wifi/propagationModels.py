...
class GetSignalRange(object):
    ...
    def logDistance(self, **kwargs):
        ...
        (dist) is the distance between the transmitter and the receiver (m)"""
        txpower = kwargs['node'].params['txpower'][kwargs['wlan']] #transmit power
        gain = kwargs['node'].params['antennaGain'][kwargs['wlan']]   #antenna gain(dbm)
        gains = txpower + (gain * 2)
        ref_d = 1

        pl = self.pathLoss(kwargs['node'], ref_d, kwargs['wlan'])   #path loss
